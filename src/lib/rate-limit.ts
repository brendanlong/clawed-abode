 ...
 *
 * Two sources feed the readings, both normalized to {@link RateLimitReading}:
 *   - `rate_limit_event` messages on a session's stream (the reactive path — the
 *     only one that reports `rejected`);
 *   - the SDK's structured `/usage` snapshot, polled at turn boundaries (the
 *     pre-emptive path — it always carries a utilization figure, whereas events
 *     are only emitted when the info changes).
 */
 ...
 * optional and the shape mirrors a server response we don't control, so unknown
 * keys are ignored and a bad payload yields `null` rather than throwing in the
 * query loop.
 */
...
/**
 * Extract a {@link RateLimitReading} from an SDK message, or `null` if it is not a
 * usable `rate_limit_event`. Windows we never hold for (see
 * {@link HOLDABLE_LIMIT_TYPES}) are dropped here so callers need not know the
 * policy. A rejection with no `resetsAt` is dated {@link UNKNOWN_RESET_HOLD_MS}
 * out, so it expires on its own and the next attempt re-learns the real reset.
 */
export function parseRateLimitEvent(message: unknown, nowMs: number): RateLimitReading | null {
  if (
  ...
  if (!info.rateLimitType || !isHoldableLimitType(info.rateLimitType)) return null;
...
/** A window from the SDK's structured `/usage` response. */
...
/**
 * The subset of `SDKControlGetUsageResponse` the pause needs. The method behind it
 * is flagged experimental by the SDK, so the response is validated rather than
 * typed and an unrecognized shape simply yields no readings.
 */
...
/**
 * Readings for every dated window in a `/usage` snapshot. `rejected` is always
 * false here — the snapshot reports consumption, not refusal; only a live
 * `rate_limit_event` can tell us the API is actually turning requests away.
 */
export function parseUsageSnapshot(snapshot: unknown, nowMs: number): RateLimitReading[] {
  const parsed = UsageSnapshotSchema.safeParse(snapshot);
  if (!parsed.success || !parsed.data.rate_limits) return [];

  const readings: RateLimitReading[] = [];
  for (const [limitType, window] of Object.entries(parsed.data.rate_limits)) {
    if (!window || !isHoldableLimitType(limitType)) continue;
    const resetsAtMs = window.resets_at ? Date.parse(window.resets_at) : NaN;
    if (Number.isNaN(resetsAtMs) || resetsAtMs <= nowMs) continue;
    readings.push({
      limitType,
      rejected: false,
...
export function mergeReading(
  ...
  byType.set(incoming.limitType, incoming);
