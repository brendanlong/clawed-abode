...
- Secrets must never reach the CLI argv — it leaks via journald, world-readable `/proc/*/cmdline`, and `runGit`'s error message. Pass MCP config via the mode-0600 workspace file rather than `options.mcpServers`, and the GitHub token via the workspace file its credential helper reads, never in a clone URL.
