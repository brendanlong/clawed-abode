...
- Secrets must never reach the CLI argv (it leaks via journald and `/proc/*/cmdline`) — pass MCP config via the mode-0600 workspace file, not `options.mcpServers`, and the GitHub token via the workspace file its credential helper reads, never in a clone URL. `run` in `worktree-manager.ts` puts the argv in its error message too.
