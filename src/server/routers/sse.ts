...
import type { PrUpdateEvent, UserInputRequestEvent } from '../services/events';
...
export const sseRouter = router({
  ...
  // Subscribe to user input requests (canUseTool waiting for response)
  onUserInputRequest: protectedProcedure
    .input(z.object({ sessionId: z.string().uuid() }))
    .subscription(async function* ({ input, signal }) {
      const events: UserInputRequestEvent[] = [];
      let resolveWait: (() => void) | null = null;
      let counter = 0;

      const unsubscribe = sseEvents.onUserInputRequest(input.sessionId, (event) => {
        events.push(event);
        resolveWait?.();
      });
      ...
      try {
        while (!signal?.aborted) {
          if (events.length > 0) {
            const event = events.shift()!;
            yield tracked(`${event.sessionId}-userinput-${counter++}`, event);
          } else {
            await new Promise<void>((resolve) => {
              resolveWait = resolve;
              const onAbort = () => resolve();
