...
- The TTS voice chooser is `VoicePicker` (search box, capped rows, nothing mounted while closed), never a `Select`: Firefox with speech-dispatcher reports ~15,000 voices and a `Select` mounts every item even when closed, which froze the page for over a minute. Any other list fed by browser-reported data needs the same treatment.
