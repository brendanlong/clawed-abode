...
- `FINDING_TEXT_BUDGET` is insurance against a shape real content doesn't take, not a fix for an observed problem — don't read it as one. The library names every string it flags in a single sentence, so a page of 1000 look-alike hosts yields one ~63k-character message, and Layer 2's 36-character placeholder inflates a page that is _nothing but_ `<!---->` about 5x. Neither survives contact with reality: measured across Hacker News, Wikipedia, a GitHub repo page and a 1.2MB Guardian homepage (three of the four with `html-comments` and `hidden-html` findings, so the rewrite path was live), sanitized output ran 0.99-1.00x the input and the longest message was 487 characters. Comments are a rounding error in a real page's bytes. Resist bounding the rewrite path itself over this: truncating tool output to defend against an input no page produces trades a real failure for a hypothetical one.
