...
- **System prompt**: base (default, or the override if enabled) + the session's public-dir note + global append + per-repo append, in that order. The note describes the session rather than setting policy, so an override keeps it.
