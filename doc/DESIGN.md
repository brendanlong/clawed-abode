...
- **Tight back-to-back tool calls** — the message list carries no blanket `space-y`; each row computes its own top margin. Two consecutive `isToolCallOnlyMessage` rows (assistant messages whose only content is tool calls — no visible text or thinking) get `mt-1` instead of the usual `mt-4`, so a run of tool calls reads as one dense block rather than a widely-spaced ladder.
- **Grouped subagent transcripts** — every message the SDK tags with a `parent_tool_use_id` belongs to a subagent (`Task` tool) turn. `groupSubagentMessages` buckets them by that id; `MessageList` **excludes** them from the top-level list (`getParentToolUseId(content) === null`) and instead renders each group, collapsed, inside its parent `TaskDisplay`. So the user sees just "a Task is running" (with the tool's own Running/Done badge) and can expand to read the subagent's full transcript. The nested transcript is produced by `renderSubagentTranscript(toolUseId)` on `MessageListContext` — a **callback** supplied by `MessageList` (which owns the `MessageBubble` import) rather than a direct import in `TaskDisplay`, avoiding a `TaskDisplay → MessageBubble` module cycle. It reuses the shared `toolResults` map and `pairedMessageIds` set so a subagent's tool calls pair with their results exactly like top-level ones. Grouping is recursive: a nested `Task` inside a subagent transcript reads the same context and renders its own children. The full transcript (including subagent messages) still streams and persists unchanged — grouping is purely a rendering concern.
