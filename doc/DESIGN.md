...
- **Crash recovery.** Scopes survive the app process, so a crash (no teardown) would orphan them. On startup (`instrumentation.ts`), `sweepSessionScopes` runs `systemctl --user stop clawed-session-*.scope` to reap any before sessions revive into fresh scopes.
- **Fail to unwrapped.** If a systemd user scope isn't available or the CLI binary can't be resolved (probed once, memoized in `getSessionScopeConfig`), the session launches the CLI normally with no scope — best-effort cleanup, never a hard requirement. The launcher itself also falls back to `exec`ing the CLI directly when its scope env / `systemd-run` are absent.

_Known limitations_: (1) an untrappable SIGKILL to the app can't run teardown, so scopes linger until the next startup sweep; (2) reaping is session-scoped, so daemons a session starts persist for that session's lifetime (by design — we don't reap per background task).
