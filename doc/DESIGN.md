...
- **GlobalSettings**: Global application settings (system prompt override and append, Claude model, advisor model, Claude API key, claude.ai session cookie + org id for usage limits, TTS speed, voice auto-send)
...
Next to the context indicator, `UsageLimitsIndicator` shows the account's **subscription usage limits** as small bars (issue #379): the **session** (5-hour) limit and the **weekly** limit. There is no public API for these; the data comes from claude.ai's internal `GET /api/organizations/{orgId}/usage`, which authenticates with the user's **claude.ai session cookie**. The feature is opt-in: it renders nothing until the cookie is configured in Settings → System Prompt → Usage Limits.

- **Settings**: `claudeAiSessionCookie` (encrypted at rest, like the Claude API key; accepts either the bare `sessionKey` value or a full cookie string) and `claudeAiOrgId` (optional — when unset the org is auto-discovered via `GET /api/organizations`, preferring the org with the `chat` capability). The settings card shows a live fetch status ("Working — session 58%, …" or the error) so a bad cookie is debuggable.
- **Server** ([`src/server/services/claude-usage.ts`](../src/server/services/claude-usage.ts)): fetches with a browser-like User-Agent (claude.ai sits behind Cloudflare), validates the response with Zod (only the `limits` array is consumed), and caches the result in memory for 60s — failures too, so a bad cookie doesn't hammer claude.ai. `getUsageLimits` never throws; failures are logged and surfaced as `{ configured, limits: null, error }`. The settings mutations clear the cache so changes apply immediately. Served by `claude.getUsageLimits` (no input — the limits are **account-global**, not per-session).
- **Selection** (pure, [`src/lib/usage-limits.ts`](../src/lib/usage-limits.ts)): `selectUsageLimits(limits, activeModel)` picks the `session`-group limit plus one `weekly`-group limit — the **model-scoped** weekly limit when its scope matches the session's active model (matched case-insensitively against the scope's model `id`/`display_name`, e.g. a "Fable" scope matches `claude-fable-5`), otherwise the all-models weekly limit. The active model comes from the already-detected `tokenUsage.model`, so the selection happens client-side and different sessions can show different weekly bars from the same cached response.
- **Client**: the session page polls `claude.getUsageLimits` every 60s (matching the server cache TTL) and passes the raw limits into `MessageList` → `UsageLimitsIndicator`, which stays a pure props component. Bar colors follow the context indicator's thresholds, bumped by the API's own `severity` when more urgent; the tooltip shows each limit's reset time.
- _Known limitation_: the claude.ai session cookie expires eventually; when it does the bars silently disappear (the error is visible in the settings card and the logs) until a fresh cookie is pasted. The API is internal and unversioned, so the Zod schema is deliberately lenient — unknown fields are ignored and a shape change degrades to hidden bars, never a crash.
...
- **Usage Limits (claude.ai session cookie + organization ID)**: Enables the subscription usage-limit bars next to the context indicator (see [Subscription Usage Limits](#subscription-usage-limits)). The cookie is stored encrypted at rest with a "configured"-only status like the API key; the org ID is optional (auto-discovered when unset).
...
- Context usage indicator, with subscription usage-limit bars beside it when a claude.ai session cookie is configured (see [Subscription Usage Limits](#subscription-usage-limits))
...
│   │   │   ├── claude-usage.ts   # claude.ai usage-limits fetch (cookie auth, org discovery, cache)
...
│   │   ├── usage-limits.ts       # claude.ai usage-limit schemas + pure session/weekly bar selection
...
│       ├── UsageLimitsIndicator.tsx # Subscription usage-limit bars (session + weekly)
│       ├── Header.tsx
