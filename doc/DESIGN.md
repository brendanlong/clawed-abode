...
- **Scope naming.** `clawed-session-<sessionId>-<nonce>.scope` (nonce per establishment, in `CLAWED_SESSION_SCOPE`). The nonce stops a stop→start / resume from colliding with a not-yet-torn-down scope of the same session; the exact name is stored on session state so teardown stops precisely that scope. A short `TimeoutStopSec` bounds the stop so a process ignoring SIGTERM can't wedge it near systemd's 90s default before the cgroup SIGKILL.
- **Crash recovery.** Scopes survive the app process, so a crash (no teardown) would orphan them. On startup (`instrumentation.ts`), `sweepSessionScopes` runs `systemctl --user stop clawed-session-*.scope` to reap any before sessions revive into fresh scopes. The sweep glob has no per-instance discriminator, so it runs **only for the production instance** — a `pnpm dev` instance as the same user must not sweep, or it would cgroup-kill the prod instance's live sessions (the deployment host runs a single production instance).

_Known limitations_: (1) an untrappable SIGKILL to the app can't run teardown, so scopes linger until the next startup sweep; (2) reaping is session-scoped, so daemons a session starts persist for that session's lifetime (by design — we don't reap per background task); (3) when a query loop exits on a **transient** SDK error and the session then revives with `resume`, the scope stop reaps the daemons the agent had started, so from the model's perspective its dev services vanish between turns with no signal — an accepted consequence of the tree being gone anyway once the CLI subprocess dies.
