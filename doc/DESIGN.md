...
- **Known tradeoff**: because tool output is scrubbed unconditionally, a task that legitimately needs to _see_ invisible/control characters (e.g. debugging a file's zero-width characters) will receive the neutralized version. The `found` categories are logged so this is observable; a per-session opt-out could be added later if it bites.
