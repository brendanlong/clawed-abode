...
2. **Global session-list stream** — `sse.onSessionListEvents()`. `emitSessionUpdate`,
   ...
   so a turn ending while a subagent (or other end-state background task) is still running does
   **not** notify (the user asked that notifications fire only when no agent — main or background
   — is running). `backgroundActive` here **excludes tasks with no knowable end state** — permanently-backgrounded
   Bash daemons and `persistent: true` Monitor watches (`taskHasEndState`, see
   [Two-Axis Status](#two-axis-status--background-tasks)): a turn that ends while only a dev
   server or a session-length PR watch runs in the background is genuinely idle and _does_
   notify — otherwise a long-lived daemon would suppress the finished notification for the
   rest of the session. We
...
2. **Visible (dedicated displays)**: `error` (the app's synthetic error, rendered by `MainMessageBubble`), `compact_boundary` (`CompactBoundaryDisplay`), and `model_refusal_fallback` (`RefusalFallbackDisplay`) render in the transcript — they carry signal the user needs. `model_refusal_fallback` marks a **silent primary→fallback model downgrade after an API refusal** (e.g. Fable→Opus when Fable's safeguards flag a request); without a visible marker the downgrade is invisible (the reply just comes back on a different model), so it renders as an amber banner showing the `original_model → fallback_model` transition, the `api_refusal_category` (cyber/bio/…), and the API's explanation. (It was previously swept into bucket 3 by the issue #312 noise-reduction pass — this restores its visibility, since a downgrade is signal, not noise.)
3. **Hidden**: every other persisted `system` subtype — `init`, `hook_started`, `hook_response`, and generic notices like `notification`, `permission_denied`, `plugin_install`, `memory_recall`, `mirror_error`, `task_started`, `task_notification` — is hidden from the transcript to reduce noise (issue #312). The pure `isHiddenSystemMessage(type, content)` ([`messageHelpers.ts`](../src/components/messages/messageHelpers.ts)) drops anything `system` whose subtype isn't in the small visible set, gating both the `MessageList` filter (no empty spacer row) and the `MessageBubble` render. These messages are **still persisted** (server-side `classifyMessage` is unchanged) — they're just not shown, so widening the visible set later needs no backfill.
...
- **background tasks** — a `Map<task_id, BackgroundTask>` driven by `task_started` (add) / `task_notification` (remove). Emitted as a latest-value `background` SSE event and read via `claude.getBackgroundTasks` (seeded once, updated by the stream, resynced on reconnect). Whether the map contains a task **with a knowable end state** (`backgroundActive`, via the pure `taskHasEndState`) is also surfaced to the session list via `sessions.list` (`isSessionBackgroundActive`), so a live-but-main-idle session with a running subagent shows the **background** badge (see [Session List](#session-list-home)); `emitBackgroundTasks` also fans a global `claude_background` signal so that badge stays live on every background-set flip, including the ones with no `running`/`finished` edge (a task settling with no continuation, or a ✕-stop). The session-view `ClaudeStatusIndicator` likewise shows "Main agent idle — a background task is still running" instead of "waiting" in that state.

  **Tasks with no knowable end state are excluded from the busy axis.** `backgroundActive` (and thus the background-vs-waiting badge **and** the "Claude finished" notification gating) counts only tasks that will settle on their own — subagents (`task_type` `local_agent`/`remote_agent`) and workflows (`local_workflow`) run to completion, and deadline-bounded Monitor watches (`monitor`) carry a hard `timeout_ms` (default 5 min, max 1 h), so a slow CI watch delays the notification by at most an hour — correctly, since when the watch fires the main agent resumes and _that_ turn's end is the real "done". Two kinds are excluded because they may never emit a `task_notification`, and counting them would pin the session "background" and suppress the finished notification forever: a **backgrounded Bash command** (`local_bash`), which may be a permanently-running daemon (a dev server, a database); and a **`persistent: true` Monitor**, which is session-length by design (its `timeout_ms` is ignored — meant for PR watches / log tails). The exclusion is the pure `taskHasEndState(task)` (`task_type !== 'local_bash' && !persistent`; unknown/absent types count, a safe default) shared by the server (`backgroundActive`, `isSessionBackgroundActive`, `emitBackgroundTasks`' global `active` signal) and the client (`useClaudeState.backgroundActive`, `ClaudeStatusIndicator`). Detecting a persistent Monitor takes one extra linkage step, since the `persistent` flag lives only on the Monitor `tool_use` block's input while `task_started` carries just the `tool_use_id`: the reducer remembers the ids of `persistent: true` Monitor calls as assistant messages fold through (`persistentMonitorToolUseIds` on `LiveStatus`, scanned from any assistant message including subagents'), and consumes the id when the matching `task_started` arrives, marking the task `persistent`. The exclusions gate **only** the busy axis — excluded tasks are still tracked in the map and still shown in the stoppable "N background tasks running" list with their ✕, so the user can see and stop them. One accepted imperfection, because a daemon is indistinguishable from a finite command at `task_started` time (the `task_type` is the best signal the SDK provides): a **finite** backgrounded Bash (a long build or test run) is also excluded, so a turn ending while one runs notifies "finished" early — self-correcting, since the task's settle makes the main agent auto-continue and that continuation's turn-end notifies again. This is an **indicator only** — it never gates input, so a user can keep chatting while a background task runs. This is genuinely interactive, not just cosmetic: a prompt sent while a background subagent runs is answered in a new turn that **interleaves** with the still-running subagent (confirmed by `scripts/spike-concurrent-send.ts` — the reply arrived ~19s before the subagent's `sleep 20` settled), not queued until it finishes. `ClaudeStatusIndicator` shows a separate "N background tasks running — you can keep chatting" line with per-task stop controls (`claude.stopBackgroundTask` → `query.stopTask`, then **optimistic removal**: the runner drops the entry from the live set itself rather than waiting for the SDK's terminal `task_notification`, so the ✕ reliably clears the indicator whether the task is still alive or a phantom whose notification was dropped — if it was real and the notification arrives later, the reducer's removal is a `has`-guarded no-op). The pure map-removal (`removeBackgroundTask` in [`session-status.ts`](../src/lib/session-status.ts)) is shared by the notification-settle path and the stop path. _Known limitation_: a task lingers in the map if **neither** a `task_notification` **nor** a user ✕ ever happens — notably a `killed` task, for which the SDK emits no `task_notification` — at which point it clears when the query is torn down. This is an accepted tradeoff (simpler than the former terminal-`task_updated` backstop): since it is indicator-only, the impact is a stale count, never a stuck composer.
...
### Session Process Reaping (cgroup)

Processes an agent starts can outlive the work that spawned them — most painfully a **daemon** that double-forks to detach (Postgres via `pg_ctl start`, Redis, a dev-server supervisor). Because it double-forks, it escapes both the process group and any `setsid` session of the command that launched it, so killing that command's process tree leaves the daemon reparented to init and running — leaking orphaned clusters (memory, ports, disk) on the shared host (issue #424). We deliberately **don't** interfere with these processes mid-session — an agent may legitimately keep a service running across many steps — we only need a guarantee that **everything a session spawned is cleaned up when the session ends**.

The mechanism is a per-session **systemd user scope** (its own cgroup). The SDK runs each session as a per-session `claude` CLI **subprocess** (all Bash tools and their daemons are its descendants), spawned via the `pathToClaudeCodeExecutable` option. `buildSdkOptions` points that option at a small **launcher** ([`SESSION_SCOPE_LAUNCHER`](../src/lib/session-scope.ts)) that `exec`s the real CLI binary inside `systemd-run --user --scope`, so the whole session tree lives in one cgroup. On teardown — `stopSession` (stop / delete / shutdown) **and** whenever the query loop exits on its own (fatal error / stream end) — the app runs `systemctl --user stop <scope>`, and a cgroup stop kills every process in the tree regardless of double-forking. The launcher forwards the CLI's args and stdio unchanged, so the SDK↔CLI stream-json protocol is unaffected (verified with a live query in [`scripts/spike-session-scope.ts`](../scripts/spike-session-scope.ts)); the launcher + scope + reaping + teardown are covered by `session-cgroup.integration.test.ts`, and the runner's teardown wiring by `claude-runner.integration.test.ts`.

Key details ([`src/server/services/session-cgroup.ts`](../src/server/services/session-cgroup.ts)):

- **Resolving the real CLI binary.** The launcher must exec the same native binary the SDK would have. It's a platform-specific optional dependency of the SDK (`@anthropic-ai/claude-agent-sdk-<platform>-<arch>[-musl]`, not hoisted to the app's `node_modules`), resolved (once, memoized) from the **SDK's own module context** (`createRequire` anchored at the app root so it works in a bundled prod build, mirroring the SDK's own `-musl` glibc check) and checked for the executable bit. Passed to the launcher via the `CLAWED_CLAUDE_BIN` env var. If it can't be resolved, the session launches the CLI the SDK's own way (no scope), so a bad guess never breaks sessions.
- **Runtime fail-to-unwrapped.** Whether a scope can actually be created is decided **inside the launcher, at spawn time, in the exact launch environment** — it creates a throwaway `systemd-run … -- true` scope first and only commits to the real `exec systemd-run` if that succeeds; otherwise it `exec`s the CLI directly. This matters because `exec` can't recover if `systemd-run` fails, and the launch env (a fresh login shell + user env vars) can differ from, or drift after, the app's start-time state (e.g. the systemd **user** session dying when an SSH login without lingering logs out). So a host lacking a usable user scope — or one that loses it mid-run — degrades to unwrapped rather than hard-failing the session.
- **Launcher file.** Written to an app-owned dir (`~/.clawed/session-launcher.sh`, mode `0700`) — not world-writable `/tmp` (avoids a symlink-clobber vector) and not `/tmp`-reaped — and re-verified/rewritten on each establishment so a deleted launcher self-heals.
- **Scope naming.** `clawed-session-<sessionId>-<nonce>.scope` (nonce per establishment, in `CLAWED_SESSION_SCOPE`). The nonce stops a stop→start / resume from colliding with a not-yet-torn-down scope of the same session; the exact name is stored on in-memory session state so teardown stops precisely that scope, **and mirrored onto the session's `sessionScope` DB column** (`persistSessionScope`) so a crash-restart can reap it (see Crash recovery). A short `TimeoutStopSec` bounds the stop so a process ignoring SIGTERM can't wedge it near systemd's 90s default before the cgroup SIGKILL.
- **Crash recovery.** Scopes survive the app process, so a crash (no teardown) would orphan them. The scope unit name is persisted on the session's `sessionScope` DB column when the query is established and cleared on clean teardown — so any row left with a non-null `sessionScope` names a scope the previous process orphaned. On startup (`instrumentation.ts`), `reapOrphanedSessionScopes` ([`claude-runner.ts`](../src/server/services/claude-runner.ts)) loads exactly those recorded names and stops each by **exact unit name** (`reapSessionScopes` → `stopSessionScope`), then clears the column. **Crucially there is no `clawed-session-*` glob**: the reap can only ever touch scopes named in _this_ instance's own DB, so — unlike the former broad `systemctl --user stop clawed-session-*.scope` sweep — it can never cgroup-kill a co-tenant instance's live sessions. This is why the reap is no longer gated to production: a `pnpm dev` instance with its **own** `DATABASE_URL` has its own session ids and scope names and can safely reap only its own orphans. (The former glob sweep, run by _anything_ sharing the user — a dev instance, a manual command, or the `session-cgroup` integration test's own call — would SIGTERM every live production session at once, exit 143; that was the actual root cause of the mass-kill incident.) The one remaining caveat is that this safety rests on instances **not sharing a `DATABASE_URL`**: two live instances on one DB would have this reap stop the other's live scopes by exact name — but a shared DB already breaks the app's single-instance model (in-memory-vs-DB session state, the message-sequence counter), so "don't share a DB across concurrent instances" is a pre-existing invariant, not one this introduces. Over-recording is harmless: a name written for a scope that ends up not created (e.g. establish aborts) just makes the reap's stop a no-op; the post-reap clear nulls exactly the names it reaped (`sessionScope in [...]`), never a blanket predicate, so a name recorded live after the load is never wrongly cleared.

_Known limitations_: (1) an untrappable SIGKILL to the app can't run teardown, so scopes linger until the next startup reap; (2) reaping is session-scoped, so daemons a session starts persist for that session's lifetime (by design — we don't reap per background task); (3) when a query loop exits on a **transient** SDK error and the session then revives with `resume`, the scope stop reaps the daemons the agent had started, so from the model's perspective its dev services vanish between turns with no signal — an accepted consequence of the tree being gone anyway once the CLI subprocess dies.
...
│   │   ├── session-scope.ts     # Pure: session scope unit name + launcher script (cgroup reaping)
