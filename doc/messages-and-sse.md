...
**Backpressure.** tRPC pulls from the subscription generator only when the response stream has room, so each subscriber's in-process buffer (`createEventQueue`) is capped: on overflow it is discarded and a `resync` event is yielded instead. On the per-session stream the server first replays persisted messages from the watermark (the client dedupes by id); the client answers `resync` with `resyncLiveQueries`, the same refetch it does on a stream error.
