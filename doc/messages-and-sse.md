...
**Session list.** `sessions.list` is keyset-paginated on `(lastActivityAt desc, id desc)` via [`src/lib/keyset-page.ts`](../src/lib/keyset-page.ts) with an explicit `select`; the home page runs the active and archived lists as two independent infinite queries. PR status is **not** fetched at list time: `detectBranchAndPr` persists it on `Session.pullRequest` at each turn end (one GitHub call per turn, not per row per view) and the resulting `session` event reaches both streams, so the list and the session header read it from the row.
