...
A turn-end write alone goes stale the moment the agent stops — a PR opened, reviewed or merged afterwards would never appear, and GitHub can't notify us (the server has no public ingress). So `sessions.list` and `sessions.get` also hand their rows to `refreshStalePullRequests`, which re-fetches the ones whose `prCheckedAt` has aged past `PR_SNAPSHOT_TTL_MS` ([`src/lib/pull-request.ts`](../src/lib/pull-request.ts)) _after_ the response is served, announcing a change over SSE. Polling what's on screen keeps the cost bounded by page size per TTL rather than per request, and `prCheckedAt` is stamped even when the lookup fails, so a repo the token can't read costs one call per TTL instead of one per view.
